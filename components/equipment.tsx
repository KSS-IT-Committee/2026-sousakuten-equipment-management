...
export default async function EquipmentCell({ id }: { id: number }) {
  ...
  const availabilityPercentage = Math.round((availableCount / equipment.quantity) * 100);
  ...
  return (
    <div className={styles.cell}>
      <Link href={`/equipment?id=${id}`} className={styles.linkArea}>
        {equipment.picture && (
          <img src={equipment.picture} alt={equipment.name} width={100} />
        )}
      ...
      <div className={styles.quantitySection}>
        <div className={styles.quantityInfo}>
          ...
          <span className={`${styles.quantityValue} ${availableCount === 0 ? styles.unavailable : availableCount <= equipment.quantity * 0.3 ? styles.warning : styles.available}`}>
            {availableCount}/{equipment.quantity}
          </span>
        </div>
        <div className={styles.progressBar}>
          <div className={styles.progressFill} style={{ width: `${availabilityPercentage}%`, backgroundColor: progressColor }}></div>
        </div>
      </div>
